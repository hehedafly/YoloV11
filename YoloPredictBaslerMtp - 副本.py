...
finalResolution = (resolution[0], resolution[1])
...
multiThread = False                                              if not useargs else args.multiThread 
...
if recordResult:
    pre_fname = videoSaveFolder + timestr
    posRecFile = open(pre_fname + "mousePosRec.txt", 'w')
    if recordPredictResult:
        out = cv2.VideoWriter(videoSaveFolder + timestr+'output.mp4', fourcc, 50.0, finalResolution)
    else:
        out = None
    if not multiThread:
...
class FrameGrabber(threading.Thread):
    ...
    def startRecord(self):
        if recordResult:
            timestr = datetime.datetime.now().strftime("%m_%d_%H%M")
            self.writer = cv2.VideoWriter(videoSaveFolder + timestr+'outputraw.mp4', fourcc, 50.0, finalResolution)
            self.record = True
...
lastframeInd:int = -1
frameIndForNoBaslerCamera: int = -1
sync = False
syncInd:int = -1
markCountPerType = 32
...
def getFrame() -> tuple[bool, np.ndarray, int]:
    global frameIndForNoBaslerCamera
    if multiThread:
        ...
        try:
            if CameraType == "basler":
                ...
                if not ret:
                    print("no camera connected")
                frameIndForNoBaslerCamera += 1
            return ret, frame, frameIndForNoBaslerCamera
        except Exception as e:
            print(e)
